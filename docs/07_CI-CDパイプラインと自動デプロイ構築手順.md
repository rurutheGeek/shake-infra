...
2. インフラ用リポジリ (iac-workspace):
   - tarakoserver上で待機しているSelf-hosted runnerがシグナルを受信。
...
```bash
ansible-playbook -i local_config/ansible/inventory.ini -e @local_config/ansible/vars.yml -e @local_config/ansible/credentials/vault.yml ansible/site.yml --tags github_runner -e "github_runner_token=取得したトークン" -e "github_repo_url=https://github.com/rurutheGeek/インフラリポジリ名"
```
...
インフラ用リポジリ（ここ）の [.github/workflows/cd_deploy.yml](../.github/workflows/cd_deploy.yml) に以下を記述する。
    ...
    steps:
      ...
      - name: Deploy ubsleepy
        ...
        run: |
          ...
          ansible-playbook -i local_config/ansible/inventory.ini -e @local_config/ansible/vars.yml -e @local_config/ansible/credentials/vault.yml ansible/site.yml --tags ubsleepy
...
取得した各種機密情報をTerraformの変数ファイル（例: iac-workspace/local_config/terraform/terraform.tfvars）に記述し、適用する。
...
[.github/workflows/ci_infra.yml](../.github/workflows/ci_infra.yml) に定義されたCIは以下のテストを自動で行います。
...
- テスト内容:
    ...
    - Terraform Planのプレビュー（PRのみ）:
        - PRコメントに `terraform plan` の結果を自動投稿し、レビュアーがインフラ変更内容を確認できるようにする（terraform-plan ジョブ）。
    - コンテナ・IaCの脆弱性スキャン:
        - Docker ComposeファイルやAnsibleが参照するイメージ設定をTrivyでスキャンし、HIGH/CRITICAL 脆弱性がある場合はCIを失敗させる（trivy-scan ジョブ）。
...
3. デフォルトルールとカスタムルールの登録:
   ...
   git secrets --add-forbidden 'iac-workspace/local_config/terraform/terraform.tfvars' # terraform.tfvars のコミットを禁止
   ```
...
- Ansible Vaultファイルの未暗号化: iac-workspace/local_config/ansible/credentials/vault.yml が平文でコミットされようとしていないか。
- Terraform変数ファイル: iac-workspace/local_config/terraform/terraform.tfvars がコミットされようとしていないか。

これらのチェックは git commit 実行時に自動的に行われ、機密情報が含まれている場合はコミットが中断されます。

## 10. pre-commit フックによるローカル開発環境のセットアップ（推奨）

git-secrets（セクション9）に代わる現代的なアプローチとして、`pre-commit` フレームワークを使用する。一つの設定ファイル（`.pre-commit-config.yaml`）で複数のチェックをコミット前に自動実行できる。

### カバーされるチェック

| フック | 内容 |
|---|---|
| `ansible-lint` | Ansible ロールのベストプラクティス違反を検出 |
| `terraform_fmt` | Terraform コードのフォーマットを自動修正 |
| `terraform_validate` | Terraform の構文エラーを検出 |
| `detect-secrets` | APIキー・パスワード等の機密情報パターンを検出 |
...
設定ファイルはリポジトリルートの [`.pre-commit-config.yaml`](../.pre-commit-config.yaml) に定義されている。

## 11. Terraform Plan の PR コメント自動投稿

Pull Request 作成時に `terraform plan` の結果が PR コメントとして自動投稿される。レビュアーはGitHub 画面だけでインフラ変更の影響を確認できる。

### 必要なシークレット（追加が必要）

以下の3つを GitHub Actions Secrets に登録する（Terraform で管理可能）。

| シークレット名 | 内容 |
|---|---|
| `CLOUDFLARE_API_TOKEN` | Cloudflare プロバイダー用 API トークン |
| `CF_R2_ACCESS_KEY_ID` | Cloudflare R2 バックエンド用アクセスキー |
| `CF_R2_SECRET_ACCESS_KEY` | Cloudflare R2 バックエンド用シークレットキー |
| `TF_BACKEND_CONFIG` | `backend.hcl` の内容（endpoint/bucket/key/region を含む） |

### `TF_BACKEND_CONFIG` シークレットの内容例
...
シークレット未設定の場合は plan ジョブが `continue-on-error: true` で動作するため、PR のマージはブロックされない。