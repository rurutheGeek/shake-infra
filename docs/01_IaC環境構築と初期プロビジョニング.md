...
Windows環境では、WSL2（Windows Subsystem for Linux 2）を使用してUbuntu環境を構築し、その内部で構成管理ツールを実行する。
...
└── terraform *
    └── main.tf *
```
### 1.1 WSL2とUbuntuのインストール（Windows環境のみ）
...
4. 再起動後、Ubuntuのターミナルが自動起動する。画面の指示に従い、UNIXユーザー名とパスワードを設定する。
    ※以降の作業はすべてUbuntuターミナル内で実行する。Linux環境を直接利用する場合は本節をスキップする。
...
mkdir -p ~/iac-workspace/ansible/{playbooks,roles,files,templates}
mkdir -p ~/iac-workspace/local_config/{ansible/credentials,terraform}
cd ~/iac-workspace
```
...
1.  Ubuntuターミナルで terraform フォルダに移動する。
    ...
    ```
2.  以下のコードを [main.tf](../iac-workspace/terraform/main.tf) として作成する。
...
4. 「ゾーン ID」および「アカウント ID」という英数字の文字列表示されているため、それぞれの右側にある「クリックしてコピー」を使用して取得する。
...
### 1.7 Ansible 性能最適化設定（ansible.cfg）

[ansible/site.yml](../iac-workspace/ansible/site.yml) を実行する前に、リポジトリの [iac-workspace/ansible.cfg](../iac-workspace/ansible.cfg) で以下の性能最適化が施されている。これらは Raspberry Pi の遅い CPU でも実行時間を大幅に短縮するため重要。

```ini
[defaults]
# Pythonインタプリタの自動検出（警告抑制）
interpreter_python = auto_silent

# 並列実行数（ホスト数3台に対して余裕を持たせた設定）
forks = 10

# ファクト収集の最適化:
#   smart = 同一プレイブック実行内で同じホストへの2回目以降の収集をスキップ
gathering = smart
# ファクトキャッシュ: 前回実行のファクトを再利用し、再実行時の収集コストを削減
fact_caching = jsonfile
fact_caching_connection = ~/.ansible/fact_cache
fact_caching_timeout = 86400

# 接続タイムアウトをデフォルトの10秒から30秒に延長
timeout = 30

[ssh_connection]
# SSHパイプライン: タスクごとにSSH接続を確立し直すコストを削減（最大の効果）
pipelining = True
# SSH接続の多重化および接続タイムアウト設定の追加
...
各設定の意図は [09_技術スタックと選定根拠.md](09_技術スタックと選定根拠.md) §9.8 に詳述。これらの最適化により、フルデプロイの実行時間は最適化前と比べて約 40〜50% 短縮されている。

### 1.8 Ansibleの構成と適用（自動操作用ユーザーの作成と初期通信）
...
[all:vars]
ansible_ssh_private_key_file=local_config/ansible/credentials/id_rsa
```
3. 自動操作専用ユーザー（ansible_admin）を作成するためのPlaybookを [playbooks/create_ansible_user.yml](../iac-workspace/ansible/playbooks/create_ansible_user.yml) として作成・保存する。
```yaml
---
...
6. 各サーバーに共通して適用する初期通信確認、セキュリティ自動アップデート、OSのライフサイクル監視、および補助記憶装置（マイクロSDカードやSSD）の摩耗・寿命低下を防ぐためのスワップ（SWAP）無効化設定を一括制御する共通設定ロール [common](../iac-workspace/ansible/roles/common) と、全体のマスタープレイブック [site.yml](../iac-workspace/ansible/site.yml) を作成・保存する。
...
[site.yml](../iac-workspace/ansible/site.yml)
```yaml
---
...
[roles/common/tasks/main.yml](../iac-workspace/ansible/roles/common/tasks/main.yml)
```yaml
---
...
### 1.9 Ansible の変数設計と暗号化機密変数（Ansible Vault）の配置テンプレート

本インフラ環境の構成管理では、パブリックに公開してよい通常のインフラ変数（IPアドレス、ドメイン名、バケット名など）と、外部に流出してはならない機密性の高い資格情報（APIトークン、シークレットキー、パスワード等）を完全に分離して管理します。機密情報管理の全体像は [03_機密情報管理.md](03_機密情報管理.md) を参照してください。

機密情報は Ansible Vault の仕組みで暗号化して管理しますが、本マニュアルに基づいて新規に環境をアセンブルする際は、以下の変数テンプレートとプレースホルダー値（ダミー）を参考に、事前に local_config/ansible/vars.yml および local_config/ansible/credentials/vault.yml を作成して配置します。

#### local_config/ansible/vars.yml の作成と配置
公開可能なパブリック変数を定義します。以下の内容で local_config/ansible/vars.yml を作成します。
...
#### local_config/ansible/credentials/vault.yml（暗号化用ダミーテンプレート）の作成と配置
機密情報のプレースホルダーです。以下の内容で local_config/ansible/credentials/vault.yml を作成します。
実際の本番適用時は、このファイルを ansible-vault encrypt local_config/ansible/credentials/vault.yml コマンドで暗号化して管理します。
