...
- [2] 構文チェック（Ansible 標準のシンタックス確認。要 Ansible Vault パスワード）
- [3] 模擬実行（Dry Run / Check mode によるシミュレーション。要 Ansible Vault パスワード）

[2] デプロイ反映フェーズ（要 Ansible Vault パスワード）
...
[3] サーバー電源・ライフサイクル操作（要 Ansible Vault パスワード）
...
| ロール名 | 対象サーバー | 主なタスク内容 |
| :--- | :--- | :--- |
| `common` | 全サーバー | `unattended-upgrades` を用いた自動セキュリティパッチ適用、および補助記憶装置の寿命保護を目的とするスワップ機能の完全無効化。 |
...
| `ups_exporter` | 電源監視ノード | `uv` によるPython仮想環境構築、I2Cバス経由でのUPS電池情報取得、systemdでのデーモン常駐化。 |
